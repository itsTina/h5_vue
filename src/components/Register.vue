<template>
  <div class="register-container">
    ...
    <ul>
    	...
    	<li><label for="J_Name">真实姓名：</label><input type="text" id="J_Name" v-model="user.name" v-on:blur="distributeType('name')" placeholder="用于就诊信息调用,务必正确填写"/></li>
        <li><label for="J_Pwd">新建密码：</label><input type="password" id="J_Pwd" v-model="user.password" v-on:blur="distributeType('pwd')" placeholder="请填写新建密码"/></li>
    	<li><label for="J_SmsCodeNumber">图片校验码：</label><input class="wh" type="text" id="J_SmsCodeNumber" v-model="user.imgcode" v-on:blur="distributeType('smsCode')" placeholder="点击图片换一换"/><span class="codeimg"><img src="" id="J_SmsValidCode" v-on:click="verifyCode"/></span></li>
    	<li><label for="J_CodeNumber">短信验证码：</label><input class="wh" type="text" id="J_CodeNumber" v-model="user.msgcode" v-on:blur="distributeType('code')" placeholder="请填写短信验证码"/><a href="javascript:;" class="msg" id="J_GetCode" v-on:click="getCode">获取验证码</a></li>
    	<li><div class="btn"><input type="submit" id="J_RegisterBtn" value="注册"/></div></li>
    </ul>
    </form>
...
export default {
  ...
  data () {
    return {
       msg: '注册',
       user:{tel: '',name: '',password: '',imgcode: '',msgcode: ''},
       currentMenuType: 'my',
       isHide:false
    }
  },
      ...
      validateCode: function($codeInput, $tip, tip){
       ...
       getImgUrl: function(){
        ...
        getCode: function () {
              ...
              if(validatePhoneFlag && smsCodeNumberFlag){
                  if(!$('#J_GetCode').hasClass('disabled-code')){
                      ...
                      var formData = JSON.stringify({tel: $.trim($('#J_Phone').val()), imgcode: $.trim($('#J_SmsCodeNumber').val()),imgid:$('#J_SmsCodeNumber').attr('data-id'),msgtype:9});

                      axios.post(getUrl(config.url_map.sendMsg), JSON.parse(formData))
                          .then(function(res) {
                              if(res.data.status.error_code === 0){
                                  var registerCookieName = 'registerRemained' + $.trim($('#J_Phone').val());
                                  Cookie.add(registerCookieName, 60, 60);//添加cookie记录,有效时间60s
                                  setTime($('#J_GetCode'), registerCookieName);//开始倒计时
                              }else{
                                  this.verifyCode();
                                  this.errorTip($('#J_ErrorTip'), res.data.status.error_msg);
        ...
        submit: function (){
          ...
          if(!checkPhoneFlag){
            this.errorTip($('#J_ErrorTip'), '手机号已注册');
            return false;
          }
          ...
          if(validatePhoneFlag && checkPhoneFlag && validateNameFlag && validatePwdFlag && validateSmsCodeFlag && validateCodeFlag){
            if(!$('#J_RegisterBtn').hasClass('disabled')){
                $('#J_RegisterBtn').addClass('disabled');

                this.user.imgid = $('#J_SmsCodeNumber').attr('data-id');

                var formData = JSON.stringify(this.user);

                axios.post(getUrl(config.url_map.register), JSON.parse(formData))
                    .then(function(res) {
                        if(res.data.status.error_code === 0){
                          setTimeout(function(){
                        ...
                        $('#J_RegisterBtn').removeClass('disabled');
                    }.bind(this))
                    .catch(function(err) {
                      console.log(err);
